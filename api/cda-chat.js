...
function isTryAgainRequest(message) {
  const text = normalizeReplyIntent(message);
  return ["prøv igen", "prov igen", "try again"].includes(text);
}
...
function isRoleplayStartRequest(message) {
  ...
  const patterns = [
    ...
    "hændelsesanalyse",
    "haendelsesanalyse",
    "analyser denne hændelse",
    "analyser denne haendelse",
    "spil en elev",
    "spil et barn",
    "spil en forælder",
    "spil en foraelder",
...
function isRoleplayDebriefRequest(message) {
  ...
  const patterns = [
    ...
    "hvad kunne jeg gore anderledes",
    "hvad hørte barnet",
    "hvad horte barnet",
    "forklar reaktionen",
    "rådgivning",
...
function isRoleplayFinalEndRequest(message) {
  const text = normalizeReplyIntent(message);
  const patterns = [
    "afslut træningen helt",
    "afslut traeningen helt",
    "stop rollespillet helt",
    "slut rollespillet helt",
    "afslut rollespillet",
    "stop træningen helt",
    "stop traeningen helt"
  ];
...
function hasConcreteRoleplayIncident(state) {
  if (!state || typeof state !== "object") {
    return false;
  }

  const situation = String(state.situation || "").trim();
  const turns = trimRoleplayTranscript(state.turns);
  const substantiveTurns = turns.filter(
    (turn) => String(turn.text || "").trim().length >= 12
  );

  return situation.length >= 12 || substantiveTurns.length >= 2;
}

function getChildRoleFromState(state) {
  const candidates = [
    state?.cda_role,
    state?.user_role,
  ...
  return (
    candidates.find((candidate) => {
      const normalized = normalizeReplyIntent(candidate);
      return childWords.some((word) => normalized.includes(word));
    }) || "barnet i hændelsen"
  );
}
...
async function runRoleplayTurn({
  ...
  const retryInput = Boolean(
    !isStarting && state?.phase === "awaiting_retry"
  );
  const debriefRequested = Boolean(
    !isStarting && !retryInput && isRoleplayDebriefRequest(message)
  );
  const finalEndRequested = Boolean(
    !isStarting && isRoleplayFinalEndRequest(message)
  );
  const continuingDebrief = Boolean(
    !isStarting &&
      !retryInput &&
      state?.phase === "debrief" &&
      !finalEndRequested &&
  ...
  const inDebrief = debriefRequested || continuingDebrief;
  const structureChangeRequested = Boolean(
    !isStarting &&
      !retryInput &&
      !inDebrief &&
      isRoleplayStructureChangeRequest(message)
  ...
  const retryChildRole = getChildRoleFromState(currentState);

  const roleLockInstructions = retryInput
    ? [
        "PRØV IGEN ER AKTIV.",
        `Brugeren taler nu som den voksne/læreren i den oprindelige hændelse. Du svarer som \"${retryChildRole}\".`,
        "Brugerens besked er den nye formulering, der skal afprøves i præcis samme konkrete øjeblik i hændelsen.",
        "Svar kun med barnets umiddelbare, naturlige reaktion eller replik.",
        "Giv ingen analyse, feedback, overskrift, forklaring, vurdering, forslag eller læringspointe i denne tur.",
        "Gentag ikke lærerens formulering. Fortsæt ikke til et nyt scenarie.",
        "Reaktionen skal bygge på den gemte hændelse, det tidligere forløb, hændelsesanalysen og eventuelt Reverse.",
        "Reaktionen skal være plausibel, men må ikke fremstilles som sikker viden om et bestemt barn.",
        "Hold status active og bevar situation, deltagere og scenario_id.",
      ].join("\n")
    : rolesAreLocked
      ? [
      ...
      : structureChangeRequested
        ? [
            ...
            "Bevar den samme konkrete situation, deltagere og kendte hændelsesfakta.",
            "Skift kun det perspektiv eller de roller, brugeren direkte beder om.",
            "Ved Reverse skal samme hændelse opleves fra den modsatte parts perspektiv; opfind ikke en ny hændelse.",
          ].join("\n")
        : "Ingen ekstra rollelås er nødvendig i denne tur.";

  const debriefInstructions = inDebrief
    ? [
        "DEBRIEF/HÆNDELSESANALYSE ER AKTIV.",
        "Træd ud af den spillede rolle og analysér hele det gemte hændelsesforløb, ikke kun den seneste besked.",
        "Skeln mellem hvad den voksne sagde eller gjorde, hvordan barnet muligvis modtog det, hvad der kan have udløst reaktionen, og hvad der konkret kan formuleres eller gøres anderledes næste gang.",
        "Vurder konkret, hvad brugerens ord gjorde godt, hvad der kunne skabe modstand eller misforståelse, og hvad næste faglige skridt er.",
        "Hvis brugeren ønsker en formulering, lever en færdig formulering med de faktiske oplysninger fra forløbet. Brug aldrig pladsholdere.",
        "Opfind ikke detaljer. Hvis en nødvendig oplysning mangler, sig præcist hvad der mangler.",
        "Hold status active, så brugeren kan stille opfølgende spørgsmål, bruge Reverse eller vælge Prøv igen.",
      ].join("\n")
    : "Ingen debrief er aktiveret i denne tur.";
  ...
  const instructions = [
    ...
    "Under aktiv træning skal du blive i den valgte rolle. Giv kun feedback undervejs, hvis feedback_mode er during eller brugeren beder om det.",
    ...
    "Ved Reverse skal du skelne mellem afsenderens hensigt, mulig oplevelse hos modtageren, risiko for misforståelse og en mulig justering.",
    "Påstå aldrig med sikkerhed, hvad et bestemt barn tænker eller føler. Brug forsigtige formuleringer i analyse; i selve træningsreplikken spiller du en plausibel reaktion.",
    "Diagnoser er kun kontekst og må aldrig gøre en reaktion fast eller stereotyp.",
    "Forstå naturlige styringsønsker som stop, giv feedback, byt roller, Reverse, start forfra, gør situationen sværere, tilføj en deltager, ingen feedback endnu, giv valgmuligheder og lad mig svare frit.",
    "En anmodning om debrief, rådgivning eller feedback afslutter ikke automatisk tilstanden.",
    "Sæt kun status til ended, når brugeren direkte afslutter træningen helt, eller når et enkeltstående beskedtjek tydeligt er færdigt.",
  ...
  const input = JSON.stringify({
    ...
    role_control: {
      roles_locked: retryInput || rolesAreLocked,
      user_role: retryInput ? role : lockedUserRole || null,
      cda_role: retryInput ? retryChildRole : lockedCdaRole || null,
      structure_change_requested: structureChangeRequested,
      debrief_active: inDebrief,
      retry_active: retryInput,
      final_end_requested: finalEndRequested,
    },
  ...
  if (response.status === "incomplete") {
    throw new Error("Ufuldstændigt svar fra CDA-træningsmotoren");
  }

  const rawResult = JSON.parse(response.output_text || "{}");
  const effectiveStatus = finalEndRequested
    ? "ended"
    : retryInput || inDebrief
      ? "active"
      : rawResult.status;
  const result = {
    ...rawResult,
    status: effectiveStatus,
    mode: retryInput ? currentState.mode : rawResult.mode,
  };
  ...
  return {
    ...
    nextState: {
      version: 1,
      mode: retryInput ? currentState.mode : result.mode,
      user_role: retryInput
        ? role
        : preserveRoleplayStructure
      ...
      scenario_id: preserveRoleplayStructure
        ...
        : result.scenario_id,
      incident_context: Array.isArray(currentState.incident_context)
        ? currentState.incident_context
        : null,
      turns,
    },
  };
...
try {
  ...
  if (isTryAgainRequest(message)) {
    if (!activeRoleplayState || !hasConcreteRoleplayIncident(activeRoleplayState)) {
      return res.status(200).json({
        success: true,
        reply:
          language === "English"
            ? "There is no active incident to retry. Start or analyse a concrete incident first."
            : "Der er ingen aktiv hændelse at prøve igen. Start eller analysér først en konkret hændelse.",
        model: "local",
        tools_used: ["dynamicRoleplayRetry"],
        tool_debug: [
          {
            name: "dynamicRoleplayRetry",
            action: "retry_without_active_incident",
          },
        ],
        pending_action: activeRoleplayState
          ? encodeRoleplayState(activeRoleplayState)
          : null,
      });
    }
    ...
    const retryPrompt =
      language === "English"
        ? "Write the new wording you want to try with the child."
        : "Skriv den nye formulering, du vil prøve over for barnet.";
    const retryState = {
      ...activeRoleplayState,
      phase: "awaiting_retry",
      incident_context: Array.isArray(activeRoleplayState.incident_context)
        ? activeRoleplayState.incident_context
        : trimRoleplayTranscript(activeRoleplayState.turns),
      turns: trimRoleplayTranscript([
        ...(activeRoleplayState.turns || []),
        { role: "user", text: message },
        { role: "assistant", text: retryPrompt },
      ]),
    };
    ...
    return res.status(200).json({
      ...
      tool_debug: [
        {
          name: "dynamicRoleplayRetry",
          action: "await_new_formulation",
          mode: activeRoleplayState.mode || null,
          scenario_id: activeRoleplayState.scenario_id || null,
        },
      ],
      pending_action: encodeRoleplayState(retryState),
    });
  }
  ...
  if (activeRoleplayState || isRoleplayStartRequest(message)) {
    ...
    console.log("CDA kommunikationsmotor:", {
      action: activeRoleplayState
        ? activeRoleplayState.phase === "awaiting_retry"
          ? "retry_formulation"
          : "continue"
        : "start",
