...
function isCdaInternalSourceRequest(message) {
  ...
  const sourceQuestionMarkers = [
    "hvor far du data fra",
    ...
    "bruger du interne data",
    "bruger du cda data",
    "bruger du internet",
    "henter du fra internettet",
    "har du adgang til interne filer",
    "kan du åbne interne filer",
    "kan du abne interne filer",
    "dine datakilder",
    "dine kilder",
  ];

  return sourceQuestionMarkers.some((marker) =>
    text.includes(normalizeReplyIntent(marker))
  );
}

function buildCdaInternalSourceReply() {
  return [
    "Ja — CDA Engine kan bruge interne CDA-datafiler og regler, når de er relevante for spørgsmålet.",
    "",
    "Det betyder ikke, at jeg kan åbne dine private filer eller dokumenter på din computer. Dem bruger jeg kun, hvis du selv deler indholdet i samtalen.",
    "",
    "I CDA-flowet kan grundlaget være:",
    "- din beskrevne case og den aktive sagskontekst",
    "- CDA_HeidiPrompt.md og data/prompt_rules.json",
    "- interne CDA-data som specialistdata, diagnose-, case-, skabelon-, PBL-, emotion- og komorbiditetsdata, når det relevante flow kalder dem",
    "",
    "Internet eller generel ekstern viden er ikke første kilde i CDA-flowet.",
  ].join("\n");
}

function isSpecialistPanelInventoryRequest(message) {
  const text = normalizeReplyIntent(message);
  ...
  const activeCaseSpecialistMarkers = [
    "hvad siger specialisterne",
    "spørg specialisterne",
    "sporg specialisterne",
    "specialistvinkel",
  ...
  if (
    activeCaseSpecialistMarkers.some((marker) =>
      text.includes(normalizeReplyIntent(marker))
    )
  ) {
    return false;
  ...
  const inventoryMarkers = [
    ...
    "list specialistpanelet",
    "liste over specialister",
    "specialistliste",
    "oversigt over specialister",
    "hvilke specialister har du",
    "hvilke specialister findes",
    "hvem er specialisterne",
  ];

  return inventoryMarkers.some((marker) =>
    text.includes(normalizeReplyIntent(marker))
  );
}

function buildSpecialistPanelInventoryReply() {
  const specialists = getSpecialistsForEngine();

  if (specialists.length === 0) {
    return "Jeg kan ikke finde specialistpanelet i den lokale CDA-specialistdata lige nu.";
  }

  const grouped = new Map();

  for (const specialist of specialists) {
    const group = String(specialist?.group || "Ukendt gruppe").trim() || "Ukendt gruppe";
    const name = String(specialist?.name || "Ukendt specialist").trim();
    const focus = String(specialist?.function || "Funktion ikke angivet").trim();

    if (!grouped.has(group)) {
      grouped.set(group, []);
    }

    grouped.get(group).push(`- ${name} — ${focus}`);
  }

  const lines = [
    "**Specialistpanelet**",
    "",
    `CDA har ${specialists.length} lokale AI-specialister i specialistpanelet.`,
    "Oversigten kommer fra intern CDA-specialistdata — ikke fra internettet.",
    "",
  ];
...
try {
  ...
  if (isCdaInternalSourceRequest(message)) {
    const reply = buildCdaInternalSourceReply();
    const usedTools = ["cdaInternalSourceContract"];
    const toolDebug = [
      {
        name: "cdaInternalSourceContract",
        source: "local_runtime_contract",
        role,
        response_style,
      },
    ];
  ...
  if (isSpecialistPanelInventoryRequest(message)) {
    const reply = buildSpecialistPanelInventoryReply();
    const usedTools = ["localSpecialistPanelInventory"];
    const toolDebug = [
      {
        name: "localSpecialistPanelInventory",
        source: "data/CDA_SpecialistPanel.json",
        specialists_count: getSpecialistsForEngine().length,
        role,
        response_style,
      },
    ];
