...
},
{
  ...
  description:
    "Henter det eksisterende CDA-specialistpanel. Brug ved behov for specialistperspektiver, tv├ªrfaglig vurdering eller r├Ñd fra specialistpanelet.",
  parameters: {
    type: "object",
    properties: {},
    additionalProperties: false,
...
function isConcreteStudentPblRequest(message) {
  ...
  const mentionsPbl =
    text.includes("pbl") ||
    text.includes("projektbaseret l├ªring") ||
...
function getPblProfileOfferReply() {
  return [
    "PBL kunne v├ªre relevant her, men jeg vil ikke foresl├Ñ et konkret projekt uden en kort elevprofil.",
    "",
    "Profilen skal blandt andet afklare alder, interesser, styrker, koncentration, st├©ttebehov, arbejdsform, sikkerhed og fagligt m├Ñl.",
    "",
  ...
  if (isOtherExperienceCaseRequest(message)) {
    ...
    if (selectedCase) {
      const caseInstructions = [
        ...
        "Svar kort og naturligt p├Ñ dansk ud fra den ene vedlagte case.",
