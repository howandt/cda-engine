...
function buildEmotionInstructions({
  ...
  return [
    ...
    "Giv korte, naturlige sætninger den voksne faktisk kan sige. Grænsen skal beskrive den konkrete adfærd og den næste handling, ikke barnets person.",
    "Alternative sætninger må ikke kræve, at barnet straks sidder roligt, får kroppen i ro eller på anden måde allerede har reguleret sig for at kunne lykkes.",
    "Når barnet er presset, må du ikke give et bredt spørgsmål som 'Hvad virker bedst for dig?'. Giv i stedet højst to konkrete, værdige muligheder, som den voksne kan gennemføre.",
    communicationAlreadyUsed
      ? "Ordene er allerede sagt. Medtag altid én kort relationel reparation, hvor den voksne tager ansvar for sin formulering uden at fjerne elevens ansvar for adfærden."
      : "Medtag kun relationel reparation, hvis brugerens situation viser, at ordene allerede er sagt.",
...
export async function runEmotionFlow({
  ...
  const communicationAlreadyUsed = ["jeg sagde", "jeg skrev"].some((pattern) =>
    normalizeText(message).includes(normalizeText(pattern))
  );
