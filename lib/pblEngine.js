...
async function runPblFlow({
  ...
  if (choiceState) {
    ...
    // 24C.10: naar laereren uddyber eller tilfoejer flere detaljer til det
    // projekt, der allerede er vist (fx varighed, daglig tid, hvem der er
    // til stede, hvilke faglige maal), matcher det ingen af ovenstaaende
    // korte svartyper (vaelg/forslag 2/nyt projekt/ja). Tidligere faldt
    // koden derfor heltud af choiceState og endte med at nulstille hele
    // profilen, som om samtalen startede forfra - laereren oplevede at
    // skulle svare paa de samme grundspoergsmaal igen og igen. I stedet
    // genbruges assessPblIntake (bygget netop til at udtraekke og
    // sammenholde det, brugeren har givet) til at flette de nye detaljer
    // ind i den profil, vi allerede kender, og det ALLEREDE viste projekt
    // praesenteres paa ny med en opdateret begrundelse - uden at glemme
    // noget af det, der er talt om.
    ...
    if (currentProject) {
      const refinedIntake = await assessPblIntake({
        openai,
        message,
        previousProfile: {},
      ...
      return buildPblResult({
        ...
        calls: [{ response: refinedIntake.response, phase: "pbl_choice_refinement" }],
        tools: ["pblEngineV1"],
