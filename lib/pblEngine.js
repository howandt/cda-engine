...
// 24C.11: en uddybning ("vi skal bruge 2 timer dagligt") og et konkret
// spørgsmål ("hvad skal tjeklisten indeholde?") kræver to helt forskellige
// svar - det ene skal flettes ind i profilen og give et opdateret forslag,
// det andet skal besvares direkte og konkret. Denne funktion skelner de to,
// ud fra spørgsmålstegn eller et indledende spørgeord.
function isPblFollowupQuestion(value) {
  const text = normalizeReplyIntent(value);
  if (!text) return false;

  if (String(value || "").trim().endsWith("?")) return true;

  return /^(hvad|hvilke|hvilken|hvilket|hvordan|hvorfor|hvornaar|hvornår|hvem|hvor)\b/.test(
    text
  );
}

async function answerPblProjectQuestion({
  openai,
  message,
  project,
  profileText,
  ...
  const instructions = [
    "Du er CDA's PBL-fagmotor og svarer en lærer, der allerede har fået vist et konkret PBL-projekt.",
    "Læreren stiller nu et konkret, praktisk spørgsmål om projektet eller eleven - besvar det direkte og konkret.",
    "Gå ud fra, at læreren IKKE selv er fagekspert i projektets emne (fx landbrug) - eleven kan være det. Giv derfor konkrete, brugbare svar, ikke kun spørgsmål tilbage.",
    "Skift ikke projekt, og gengiv ikke hele projektbeskrivelsen igen - svar kun på det, der bliver spurgt om.",
    "Svar kort, konkret og på dansk, gerne som en kort liste, hvis det passer til spørgsmålet.",
  ].join("\n");
...
async function refinePblProjectPitch({
  openai,
  message,
  project,
  existingReason,
  ...
  const instructions = [
    "Du er CDA's PBL-fagmotor. Læreren har allerede fået vist et konkret PBL-projekt og kommer nu med en uddybning, et ønske om en anden rækkefølge, et andet udgangspunkt eller lignende justering.",
    "Din opgave er at opdatere præsentationen af DET SAMME projekt, så den tydeligt afspejler læreren nye ønske - skift ikke til et andet projekt.",
    "Hvis læreren fx beder om at starte mere basalt/enklere, eller i en anden rækkefølge end de viste aktiviteter, SKAL de 3 nye aktiviteter afspejle det - ikke bare gentage de gamle.",
    "Skriv præcis 3 aktiviteter, der reelt tager hensyn til det, læreren har bedt om. Hvert punkt højst 12 ord.",
    "Skriv en kort 'hvorfor det passer'-begrundelse (1-2 sætninger), der nævner den nye justering.",
    "Gå ud fra, at læreren ikke selv er fagekspert i projektets emne - eleven kan være det.",
  ].join("\n");
  ...
  const input = JSON.stringify({
    project: {
      ...
      subtitle: project.subtitle || "",
      description: project.description || "",
      original_activities: project.activities || [],
    },
  ...
  return {
    reason: parsed.reason || existingReason || "",
...
// 24C.13: dette spoergsmaal blev tidligere hentet fra en fast, hardkodet
// JS-tekststreng ("PBL kan vaere en meningsfuld laeringsvej gennem X. Hvilke
// materialer eller vaerktoejer..."), som blev gentaget ordret hver eneste
// gang sikkerhedstjekket nedenfor sloeg til - uanset samtalens forloeb.
// Selve sikkerhedstjekket er legitimt (AI'en maa ikke selv paastaa, at
// opsyn/sikkerhed er bekraeftet uden reelt belaeg i det, laereren har
// skrevet), men spoergsmaalet skal formuleres frit af AI'en hver gang, ikke
// tvinges til én fast saetning.
async function generateSafetyFollowupQuestion({ openai, profile, message }) {
  const instructions = [
    "Du er CDA's PBL-intakemotor.",
    "Der mangler konkret, udtrykkeligt oplyst information om materialer, værktøj eller opsyn, før et PBL-forslag om dette emne kan gives forsvarligt.",
    "Skriv ét kort, naturligt formuleret spørgsmål på dansk, der beder om denne information.",
    "Spørgsmålet skal tage udgangspunkt i elevens konkrete interesse/emne, hvis den er kendt - men skal IKKE bruge en fast, genkendelig indledningssætning. Formuler det frit og naturligt, som en fagperson ville spørge i en almindelig samtale.",
    "Gentag ikke ordret en sætning, du har brugt i et tidligere svar i denne samtale.",
  ].join("\n");

  const input = JSON.stringify({
    known_interest: profile?.interests || "",
  ...
  const parsed = JSON.parse(response.output_text || "{}");
  return parsed.question || "Hvilke materialer eller værktøjer skal eleven bruge, og hvilket opsyn kræver det?";
}
...
async function assessPblIntake({
  ...
  const instructions = [
    ...
    "Før projektmatch skal du kende elevens alder eller klassetrin, den konkrete interesse og et fagligt læringsmål.",
    "Almindelig klasseundervisning med en lærer til stede er i sig selv tilstrækkelig voksenstøtte - antag det som opfyldt, medmindre brugeren selv nævner en anderledes eller mere krævende ramme (fx værktøj, maskiner eller arbejde uden opsyn). Spørg ikke rutinemæssigt ind til voksenstøtte i en almindelig skolesammenhæng.",
    "Hvis noget nødvendigt mangler, spørg kort efter højst to nært beslægtede oplysninger. Spørgsmålet skal lyde sikkert og fagligt, ikke tøvende. Forklar kort HVORFOR du spørger - nævn direkte hvilke(n) oplysning(er) der mangler for at kunne lave det bedst mulige forslag, så brugeren forstår, hvad der skal til, og kan svare på det hele på én gang i stedet for at blive spurgt ad flere omgange.",
    "Formulér spørgsmålet frit og naturligt hver gang - brug ikke en fast, genkendelig indledningssætning, som gentages ordret på tværs af svar. Variation i formuleringen er vigtigere end en fast skabelon.",
  ...
  if (!safetyInformationIsSupported) {
    ...
    if (assessment.status === "ready") {
      ...
      assessment.next_question = await generateSafetyFollowupQuestion({
        openai,
...
async function runPblFlow({
  ...
  if (intakeState) {
    ...
    // om at komme videre nu og tilpasse senere. Efter ét opfoelgende
    // spoergsmaal (dvs. andet forsog i alt) fortsaetter vi til match
    // med det, vi har, i stedet for at spoerge igen.
    const forceProceed =
      intake.assessment.status === "needs_info" && roundSoFar >= 1;
  ...
  if (choiceState) {
    ...
    // 24C.10/24C.12: naar laereren uddyber, tilfoejer detaljer eller beder om
    // en anden raekkefoelge/vinkel paa det projekt, der allerede er vist,
    // matcher det ingen af ovenstaaende korte svartyper (vaelg/forslag
    // 2/nyt projekt/ja). Tidligere faldt koden derfor heltud af choiceState
    // og endte med at nulstille hele profilen (24C.10-fix), og selv efter
    // den foerste rettelse blev kun profilen i baggrunden opdateret - den
    // synlige tekst (begrundelse og aktiviteter) forblev ordret uaendret,
    // saa et sagligt, konkret oenske om fx en anden raekkefoelge saa ud som
    // om det blev ignoreret. Nu opdateres baade baggrundsprofilen (til
    // senere brug, fx naar planen skal bygges) OG selve den synlige
    // begrundelse/aktivitetsliste, saa svaret reelt afspejler det, laereren
    // bad om.
    const currentProject = choiceState.shown === 3
      ? choiceState.customProject
      : choiceState.shown === 2 ? secondProject : firstProject;

    if (currentProject && isPblFollowupQuestion(message)) {
      const questionAnswer = await answerPblProjectQuestion({
        openai,
        message,
        project: currentProject,
        profileText: choiceState.profile,
      });

      return buildPblResult({
        reply: questionAnswer.answer,
        pendingAction: encodePblChoiceState(choiceState),
        calls: [{ response: questionAnswer.response, phase: "pbl_choice_followup_question" }],
        tools: ["pblEngineV1"],
        debug: [{ action: "answer_question_about_shown_project" }],
      });
    }

    if (currentProject) {
      const existingReason = choiceState.shown === 2
        ? choiceState.secondReason
        : choiceState.firstReason;

      const [refinedIntake, refinedPitch] = await Promise.all([
        assessPblIntake({
          openai,
          message,
          previousProfile: {},
          previousSource: [choiceState.profile, message].filter(Boolean).join("\n"),
          activeCaseContext,
        }),
        refinePblProjectPitch({
          openai,
          message,
          project: currentProject,
          existingReason,
      ...
      const refinedProfileText = formatCanonicalPblProfile(
        refinedIntake.assessment.profile
      );
      const updatedProject = {
        ...currentProject,
        activities: refinedPitch.activities,
      };

      const updatedState = encodePblChoiceState({
        ...choiceState,
        profile: refinedProfileText,
        ...(choiceState.shown === 2
          ? { secondReason: refinedPitch.reason }
          : { firstReason: refinedPitch.reason }),
        ...(choiceState.shown === 3 ? { customProject: updatedProject } : {}),
      });

      return buildPblResult({
        reply: formatPblChoice(
          updatedProject,
          choiceState.shown === 2 ? 2 : 1,
          refinedProfileText,
          refinedPitch.reason
        ),
        pendingAction: updatedState,
        calls: [
          { response: refinedIntake.response, phase: "pbl_choice_refinement" },
          { response: refinedPitch.response, phase: "pbl_choice_pitch_refinement" },
        ],
        tools: ["pblEngineV1"],
        debug: [{ action: "refine_shown_project_with_new_details" }],
      });
    }
  }
