...
function findLocalPblSignals(message, diagnosisMeta) {
  if (!diagnosisMeta) return [];

  const diagnosisTerm = diagnosisMeta.navn || diagnosisMeta.id || "";
  const diagnosisFiltered = getPblProjects({ diagnosis: diagnosisTerm });
  const pool = Array.isArray(diagnosisFiltered?.projects)
    ? diagnosisFiltered.projects
    : [];
  ...
  // Bevidst STRENGERE end getPblProjects({ search }): fri prosa-tekst
  // indeholder mange almindelige ord, som fuzzy-matching (stavefejl-
  // tolerance) let fejlmatcher (fx "stille" mod "lille"). Her kraeves
  // derfor et praecist, ikke-fuzzy match, og kun mod titel og
  // karriereretning - ikke mod lange, generiske beskrivelsestekster.
  const matched = pool.filter((project) => {
    ...
    return candidateWords.some((candidate) =>
      searchableWords.some(
        (textWord) =>
          textWord.includes(candidate) || candidate.includes(textWord)
      )
    );
  });
