...
function isDirectLocalCaseRequest(message) {
  ...
  // 24C.8: samme fejl som i isLocalCaseNavigationRequest (se 24C.7) - et
  // mønster som "case" blev kun genkendt, hvis der var mellemrum på BEGGE
  // sider i beskeden. Et helt normalt "...børnehave-overgangs-case", hvor
  // "case" står som allersidste ord, har intet mellemrum bagefter og blev
  // derfor aldrig genkendt. Brug containsDiagnosisPhrase (ord-grænse-sikker,
  // fungerer korrekt uanset om ordet står i start, midt eller slut af
  // beskeden) i stedet for den manuelle mellemrums-tjekning.
  return directPatterns.some((pattern) => containsDiagnosisPhrase(text, pattern));
}
...
function isLocalCaseNavigationRequest(message) {
  ...
  // 24C.7: denne funktion blev tidligere kun udløst, hvis hele beskeden var
  // identisk med ét af nøgleordene (fx kun "næste" og intet andet). En helt
  // naturlig formulering som "vis mig den næste case" blev derfor IKKE
  // genkendt, selvom en aktiv case-søgning var i gang, og samtalen faldt i
  // stedet tilbage til almindelig chat uden adgang til søgeresultatet. Brug
  // i stedet containsDiagnosisPhrase (samme ord-grænse-sikre metode som
  // resten af filen bruger), så nøgleordet også genkendes inde i en almindelig
  // sætning - ikke kun når det står helt alene.
  return [
    "naeste",
    "neste",
    "forrige",
    ...
    "find en anden case",
    "vis en anden case",
  ].some((pattern) => containsDiagnosisPhrase(text, pattern));
}
...
function getLocalCaseNavigationDirection(message) {
  ...
  if (
    containsDiagnosisPhrase(text, "forrige") ||
    containsDiagnosisPhrase(text, "tilbage")
  ) {
    return -1;
  }
