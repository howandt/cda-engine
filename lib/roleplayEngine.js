...
function isRoleplayTriggerMessage(message, config = getRoleplayConfiguration()) {
  ...
  if (isStandaloneScenarioId(message) && hasRequestedScenario(message)) {
    return true;
  }
...
function isScenarioLibraryRequest(
  ...
  return (
    ...
    (isStandaloneScenarioId(message) && hasRequestedScenario(message))
  );
}

function isStandaloneScenarioId(message) {
  return /^[a-z0-9]+(?:_[a-z0-9]+)+$/i.test(
    String(message || "").trim()
  );
}
...
function hasRequestedScenario(message) {
  const library = getRollespil();
  return Boolean(findRequestedScenario(message, library.data));
}
