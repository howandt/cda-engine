...
async function extractRolesWithAI(openai, message) {
  const text = cleanText(message, 1200);
  ...
  try {
    ...
    if (response.status === "incomplete") {
      console.error("CDA rollespil rolleudtræk: ufuldstændigt svar fra AI", {
        message: text,
      });
      return { user_role: "", cda_role: "" };
    }
...
async function runRoleplayChatRequest({ openai, body: requestBody = {} } = {}) {
  try {
    ...
    let action = detectAction(message, state, body.action);
    ...
    if (action === "start") {
      ...
      const entryMode = state.entry_mode || getRoleplayEntryMode(
        state.scene || message,
        config
      );
      const preparationQuestions =
        config?.entry_modes?.[entryMode]?.preparation_questions?.[language] || [];
      ...
      if (
        ...
        return createRoleplayResult(200, {
          success: true,
          reply: preparationQuestions.join("\n\n"),
      ...
      const roleContext = continuingPreparation
        ? [
            state.scene,
            "",
            "Brugerens svar på forberedelsesspørgsmålene:",
            message,
          ].join("\n")
        : message;
      let startUserRole =
        cleanText(body.user_role || state.user_role || body.role, 160) ||
        extractRole(roleContext, "user");
      let startCdaRole =
        cleanText(body.cda_role || state.cda_role, 160) ||
        extractRole(roleContext, "cda");

      if (!startUserRole || !startCdaRole) {
        const aiRoles = await extractRolesWithAI(openai, roleContext);
        console.log("CDA rollespil AI-rolleudtræk:", {
          message: roleContext,
          regex_fandt: { user: startUserRole, cda: startCdaRole },
          ai_fandt: aiRoles,
        });
      ...
      state = sanitizeState({
        ...
        scene: continuingPreparation
          ? cleanText(
              `${state.scene}\n\nForberedelsessvar:\n${message}`,
              6000
            )
          : cleanText(body.scene, 6000) || message,
        incident_case: "",
        history: [],
        role_events: [],
        last_analysis: "",
