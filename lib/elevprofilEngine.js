function normalizeStudentProfileText(value) {
  return String(value || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function isStudentProfileRequest(message) {
  const text = normalizeStudentProfileText(message);
  ...
  return profilePatterns.some((pattern) =>
    text.includes(normalizeStudentProfileText(pattern))
  );
}
...
function getReadableStudentProfileIntent(message) {
  ...
  if (
    [
      ...
      "hvad er naeste skridt",
      "naeste skridt ud fra profilen",
    ].some((pattern) => text.includes(normalizeStudentProfileText(pattern)))
  ) {
