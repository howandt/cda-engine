...
function readSpecialists() {
  ...
  cachedSpecialists = Array.isArray(data?.specialists) ? data.specialists : [];
  return cachedSpecialists;
}
...
function buildLocalPprAngleReply(caseData, activeContext) {
  const title = limitSpecialistTextFromEngine(
    caseData?.titel || caseData?.title || caseData?.id || 'Aktiv sag',
    10000
  );
  const problem = limitSpecialistTextFromEngine(
    caseData?.problem ||
...
async function runDirectSpecialistPanel({
  ...
  if (
    requestedAngle === 'ppr' &&
    hasActiveSpecialistContext(activeCase, activeContext)
  ) {
    return buildLocalResult({
      reply: buildLocalPprAngleReply(activeCase, activeContext),
      usedTools: ['localPprAngleOnActiveCase'],
      toolDebug: [
        {
...
export async function runSpecialistFlow({
  ...
  if (activeCase && isLocalPprCaseAngleRequest(message)) {
    return buildLocalResult({
      reply: buildLocalPprAngleReply(activeCase, activeContext),
      usedTools: ['localActiveCasePprAngle'],
      toolDebug: [
        {
          name: 'localActiveCasePprAngle',
          selected_case_id: activeCase.id || null,
          followup: message,
