...
export const SPECIALIST_ENGINE_VERSION = '24B.14-hidden-secondary-router';
...
export function getTargetedSpecialistPanelFromEngine({
  ...
  const text = normalizeSpecialistPhrase(`${message} ${extraText}`);
  ...
  const scored = candidateSpecialists.map((specialist, index) => {
    ...
    for (const pattern of anglePatterns[angle] || []) {
      if (keywords.some((keyword) => keyword === normalizeSpecialistPhrase(pattern))) {
        score += 40;
      }
    }
