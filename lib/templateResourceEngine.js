...
function searchWordMatches(searchWord, textWord) {
  ...
  // 24B.20P: samme mønster som tabel-fixet - et kort ord (fx "vis") kan
  // tilfældigt indgå i et helt andet, længere ord (fx "henvisning").
  // Et rent substring-match kræver derfor mindst 4 tegn i det korte ord,
  // mens præcis lighed altid er tilladt uanset længde.
  // 24B.20T: samme moenster igen, men nu med et helt almindeligt kort ord
  // ("elev") som tilfaeldigvis er en aegte forstavelse til et andet,
  // specifikt sammensat ord ("elevprofil"). Fire tegn var for lavt til at
  // adskille et generisk ord fra et sammensat fagudtryk - haev til 6.
  if (
  ...
  // 24B.20O: et rent 4-tegns-loft på det fælles forled matchede fejlagtigt
  // sammensatte ord som "mestringsmappe" og "mestringsprofil" (fælles
  // forled "mestrings", men helt forskellig betydning). Kræv derfor at det
  // fælles forled dækker en stor del af det korteste ord - det bevarer
  // bøjede former (som altid deler hele grundordet) uden at sammenblande
  // forskellige sammensatte ord med samme forled.
  ...
  // 24C.1: en løs "statistisk lighed"-fuzzy (Levenshtein-afstand) mellem
  // helt forskellige ord blev tidligere accepteret som match, hvis begge
  // ord var 5+ tegn og lå inden for 2 bogstavændringer af hinanden. Det
  // betød at almindelige, betydningsløse ord i fri tale (fx "taler" vs.
  // nøgleordet "faser", eller "være" vs. et andet nøgleord) blev
  // fejlagtigt regnet som et sikkert skabelon-match og kunne dermed kapre
  // en igangværende samtale. Reglerne ovenfor (eksakt match, substring på
  // mindst 6 tegn, og fælles forled på mindst 6 tegn/75% af det korteste
  // ord) dækker allerede ægte danske bøjningsformer. Et match kræver
  // derfor nu, at ordene reelt er samme ord eller en bøjning af det -
  // ikke blot statistisk tætte, forskellige ord.
  return false;
}
...
function getTemplateRequestSignals(message, templates) {
  ...
  // 24C.4: et rent ordniveau-match på et skabelon-nøgleord
  // ("knownTemplateMention"/"allowedKnownTemplateMention") er ikke i sig
  // selv et bevis på, at brugeren rent faktisk beder om en skabelon - det
  // kan lige så godt være et almindeligt ord i en fri samtale, der
  // tilfældigvis også indgår i en skabelons titel eller nøgleord. Kun et
  // eksplicit skabelon-/bank-kald eller en tydeligt formuleret indirekte
  // forespørgsel ("har I noget jeg kan bruge til...") må derfor åbne
  // skabelonen direkte her. Et rent ordmatch alene fanges ikke af denne
  // hårde routing og kan i stedet dukke op som et blødt, ikke-påtrængende
  // forslag via Heidis egen loadTemplateSuggestions.
  return {
    text,
    directBankRequest,
    ...
    listRequest,
    isDirectRequest: explicitTemplateRequest,
    isTemplateRequest: explicitTemplateRequest || indirectResourceRequest,
  };
}
...
function findBestLocalTemplate(message, templates) {
  ...
  const ignoredWords = new Set([
    ...
    "and",
    "the",
    // 24C.2: almindelige danske pronominer, biord og forbindelsesord.
    // Disse er så hyppige i fri tale, at de ikke bærer noget reelt
    // emnesignal - men de er 3+ tegn og var derfor tidligere gyldige som
    // "match-ord" mod en skabelons nøgleord, blot fordi et af skabelonens
    // egne nøgleord (fx "hvis jeg føler") tilfældigvis indeholdt samme ord.
    "jeg",
    "du",
    "han",
    "hun",
  ...
  const scoreTemplate = (template) => {
    ...
    const isMeaningfulWord = (word) =>
      Boolean(word) && word.length >= 3 && !ignoredWords.has(word);

    const titleWords = new Set(
      title.split(" ").filter(isMeaningfulWord)
    );
    const triggerWords = new Set(
      triggers.join(" ").split(" ").filter(isMeaningfulWord)
    );
    const keywordWords = new Set(
      keywords.join(" ").split(" ").filter(isMeaningfulWord)
    );
    const tagWords = new Set(
      tags.join(" ").split(" ").filter(isMeaningfulWord)
    );
    const categoryWords = new Set(
      `${category} ${subcategory}`.split(" ").filter(isMeaningfulWord)
    );
    const descriptionWords = new Set(
      description.split(" ").filter(isMeaningfulWord)
    );
