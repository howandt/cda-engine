...
export function shouldBlockTemplateAutoRouting({ message, activeCaseContext = null } = {}) {
  ...
  const hasCaseContext = hasActiveCaseContext(activeCaseContext) || isCaseContinuationLike(message);

  if (!hasCaseContext) return false;

  // 24C.3: tidligere kunne et "sikkert, navngivet" skabelon-match (fx via
  // getLocalTemplateRequest) tilsidesætte denne blokering, selv midt i en
  // aktiv sag/samtale - ud fra en antagelse om at et sådant match altid var
  // et bevidst, eksplicit ønske. I praksis kunne et enkelt tilfældigt ord i
  // en almindelig sætning udløse et "sikkert" match og dermed kapre
  // samtalen (se 24C.1). En aktiv sag/samtale må derfor nu kun forlades for
  // et eksplicit skabelon-kald (håndteret ovenfor af isExplicitTemplateRequest)
  // - aldrig blot fordi et ord i beskeden tilfældigvis matcher en skabelon.
  return true;
}
