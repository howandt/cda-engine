...
function loadEmotionSignals(message, activeCaseContext) {
  const data = readJsonIfExists(
    path.join(process.cwd(), "data", "CDA_Emotionengine.json"),
    {}
  );
  ...
  for (const [category, info] of Object.entries(data.word_categories || {})) {
    const words = Array.isArray(info?.words) ? info.words : [];
    const hits = words
      .filter((word) => text.includes(normalizeText(word)))
      .slice(0, 8);
  ...
  return {
    ...
    matched_categories: categories.slice(0, 8),
    communication_tips: Array.isArray(data.communication_tips)
      ? data.communication_tips.slice(0, 5)
      : [],
  };
}
...
function buildHeidiFlowDataContext({ message, activeCaseContext, responseStyle }) {
  ...
  const emotionSignals = loadEmotionSignals(message, activeCaseContext);
...
function buildHeidiFlowInstructions({
  ...
  const styleInstruction =
    responseStyle === "Kort"
      ...
      : responseStyle === "Dyb"
        ? "Giv dybere lærerrettet indsigt, men skriv stadig som Heidi i samtale — ikke som en klinisk manual eller et bogkapitel."
  ...
  return [
    ...
    "Skabeloner må ikke åbnes automatisk. Du må højst nævne et relevant skabelonmatch kort som mulighed. Vis kun skabelonens indhold, hvis brugeren tydeligt beder om det.",
...
export async function runHeidiFlow({
  ...
  const response = await openai.responses.create({
    ...
    instructions,
    input,
    max_output_tokens: responseStyle === "Dyb" ? 1100 : responseStyle === "Kort" ? 650 : 800,
  });
