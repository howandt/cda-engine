...
export function shouldBlockTemplateAutoRouting({ message, activeCaseContext = null } = {}) {
  ...
  // 24B.20E: en aktiv sag i samtalen må ikke forhindre et sikkert, navngivet
  // match på en eksisterende skabelon i at blive fundet - blokeringen er kun
  // til for at undgå at almindelig sagssnak fejlagtigt åbner skabelonbanken.
  try {
    const knownMatch = getLocalTemplateRequest(message);
    if (knownMatch?.type === "match" && knownMatch.isKnownTemplateMatch) {
      return false;
    }
  } catch (error) {
    console.error("Kunne ikke tjekke for sikkert skabelon-match:", error.message);
  }
