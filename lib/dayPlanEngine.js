...
function isDayPlanFollowupRequest(message, activeCaseContext) {
  if (activeCaseContext?.conversation_mode !== "day_plan") {
    return false;
  }
...
async function buildPracticalDayPlanReply({
  ...
  const instructions = [
    ...
    "DEL 1 - BARNETS DAGSPLAN: kun det barnet selv skal se eller få vist. Brug overvejende ikoner/emojis og meget få, korte ord (typisk 1-3 ord pr. trin). Mange børn husker grafik bedre end tekst, og nogle børn kan slet ikke læse endnu - planen skal give mening ud fra ikonerne alene. Ingen varsel-tider, voksen-strategi eller forklaringer i denne del.",
