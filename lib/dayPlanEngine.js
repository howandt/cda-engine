...
async function buildPracticalDayPlanReply({
  ...
  const instructions = [
    ...
    "Byg en kort, kopierbar, visuel dagsplan med korte trin og ikoner/emojis - klar til at printes eller hænges op.",
    `Fokus for denne dagsplan: ${modeLabel}.`,
    caseSummary
      ? "Der er konkrete oplysninger om barnet nedenfor (SAGSOPLYSNINGER). Tilpas rækkefølge, sprog, pauser og støtte til disse - opfind ikke yderligere diagnoser eller detaljer ud over det oplyste."
      : "Der er ingen konkret sag åben. Byg en solid, alderssvarende standarddagsplan for målgruppen, uden at opfinde en bestemt diagnose eller et bestemt barn.",
    "Brug korte trin (verb + få ord), klokkeslæt kun hvor det giver mening, og ét ikon/emoji pr. trin.",
    "Afslut altid med et kort, konkret afsnit om hvad den voksne og barnet gør, HVIS planen ændrer sig undervejs.",
    "Skriv selve dagsplanen i en ``` text ``` kodeblok, så den er let at kopiere. Før kodeblokken: én kort indledningssætning. Efter kodeblokken: én kort sætning om at tilpasse tider/aktiviteter til barnet.",
    role === "Forælder"
...
async function buildParentDayPlanMessageReply({
  ...
  const instructions = [
    ...
    caseSummary
      ? "Der er konkrete oplysninger om barnet nedenfor (SAGSOPLYSNINGER). Tilpas eksemplerne og formålet, så de passer til barnets konkrete situation - opfind ikke yderligere diagnoser eller detaljer ud over det oplyste."
      : "Der er ingen konkret sag åben. Skriv en god, generel version af beskeden.",
